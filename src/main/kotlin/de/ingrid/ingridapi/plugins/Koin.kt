...
fun Application.configureKoin() {
    ...
    val elasticHttps = environment.config.property("ktor.elasticsearch.https").getString().toBoolean()
    ...
    install(Koin) {
        slf4jLogger()
        properties(mapOf("elasticHost" to elasticHost, "elasticPort" to elasticPort, "elasticHttps" to elasticHttps, "elasticUsername" to elasticUsername, "elasticPassword" to elasticPassword))
        modules(appModule)
    }
}
...
val appModule = module {
    single(createdAtStart = true) {
        ElasticsearchService(getProperty("elasticHost"), getProperty("elasticPort"), getProperty("elasticHttps"), getProperty("elasticUsername"), getProperty("elasticPassword"))
    }
}
